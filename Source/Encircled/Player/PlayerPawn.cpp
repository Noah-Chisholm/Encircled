...
#include "Encircled/Utility/Interfaces/InteractableInterface/InteractableInterface.h"
...
{	
	...
	CrouchHeight = NormalHeight * crouchHeightMult;
	Move->SetCrouchedHalfHeight(CrouchHeight);
	SlideHeight = CrouchHeight * slideHeightMult;
...
void APC_ROTLA::Move(const FInputActionValue& Value)
{
	...
	MakeNoise(HasFlag(EEntityFlags::IsCrouching) ? LoudnessMult * CrouchNoiseMult : LoudnessMult, this, GetActorLocation(), HasFlag(EEntityFlags::IsCrouching) ? NoiseRange * CrouchNoiseMult : NoiseRange, HasFlag(EEntityFlags::IsCrouching) ? "Footsteps_Muffled" : "Footsteps");
...
void APC_ROTLA::Landed(const FHitResult& Result) {
	Super::Landed(Result);
	MakeNoise(HasFlag(EEntityFlags::IsCrouching) ? LoudnessMult * CrouchNoiseMult : LoudnessMult, this, GetActorLocation(), HasFlag(EEntityFlags::IsCrouching) ? NoiseRange * CrouchNoiseMult : NoiseRange, HasFlag(EEntityFlags::IsCrouching) ? "Falling_Muffled" : "Falling");
